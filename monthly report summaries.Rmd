...
report_doc <- read_docx() # creates an rdocx object for output to a .docx file
```
...
The narrative portions of the report should written in PLAIN TEXT using a text editor (e.g., `Notepad++`, `TextWrangler`) and then pasted into the appropriate sections of the report document. Tables and figures exported from `R` (see below) should also be copied and pasted into the appropriate sections of the report. The report should be saved with under the filename indicated below: 
...
> NOTE: Reports must be done based on information already entered, organized, and cleaned in the database! The `R` code included below works on queries or tables exported from the database in `.xlsx` format.
...
2.1 - Narrative Section - Section of text describing the main highlights of the team's activities and observations during the month, e.g., what areas the groups under study were using, comments on the general weather during the month, general observations about sexual activity, births, infant behavior, etc.
...
### Generating Section 1 - Header Section

Setting up observers, dates, taxa of interest, etc.

``` {r header info}
# Clear header information and vectors used for limiting observers, date ranges, and taxa from R
# A warning is given if these variable are not already present
...
# This will not work when knitting, so is not recommended
# os <- read_xlsx("observer samples.xlsx", guess_max = 10000)
...
report_doc <- report_doc %>%
	...
	body_add_par("") %>%
	body_add_par(paste0("Date Reported: ", reportDate)) %>%
	body_add_par("") %>%
	body_add_par(paste0("Contributors: ", contributorsList)) %>%
	body_add_par("")
```

### Generating Section 2.1 - General Summary: Narrative Section

``` {r general summary: narrative}
report_doc <- report_doc %>%
...
print(report_doc, target = "~/Desktop/report example.docx")
```
